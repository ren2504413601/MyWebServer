...
void HttpData::handleRead() 
{
    ...
    do 
    {
        ...
        printf("Request: %s", inBuffer_.c_str());
        ...
        if (state_ == STATE_PARSE_URI) 
        {
            ...
            else if (flag == PARSE_URI_ERROR) 
            {
                ...
                printf("FD = %d,%s ******", fd_, inBuffer_.c_str());
                inBuffer_.clear();
                error_ = true;
                handleError(fd_, 400, "Bad Request");
                break;
    ...
    if (!error_) 
    {
        ...
        else if (!error_ && connectionState_ != H_DISCONNECTED) events_ |= EPOLLIN;
    }
}
...
URIState HttpData::parseURI() 
{
    ...
    else 
    {
        ...
        else 
        {
            ...
            if (ver == "1.0") HTTPVersion_ = HTTP_10;
            else if (ver == "1.1") HTTPVersion_ = HTTP_11;
...
AnalysisState HttpData::analysisRequest() 
{
    ...
    else if (method_ == METHOD_GET || method_ == METHOD_HEAD) 
    {
        ...
        else filetype = MimeType::getMime(fileName_.substr(dot_pos));
        ...
        if (fileName_ == "favicon.ico") 
        {
            ...
            header += "Server: My own's Web Server\r\n";
...
void HttpData::handleError(int fd, int err_num, string short_msg) 
{
    ...
    body_buff += "<html><title>注意~出错了</title>";
    body_buff += "<body bgcolor=\"ffffff\">";
    body_buff += to_string(err_num) + short_msg;
    body_buff += "<hr><em> My own's Web Server</em>\n</body></html>";

    header_buff += "HTTP/1.1 " + to_string(err_num) + short_msg + "\r\n";
    header_buff += "Content-Type: text/html\r\n";
    header_buff += "Connection: Close\r\n";
    header_buff += "Content-Length: " + to_string(body_buff.size()) + "\r\n";
    header_buff += "Server: My own's Web Server\r\n";
    ;
    header_buff += "\r\n";
    // 错误处理不考虑writen不完的情况
